...
class UberServiceTest extends AnyWordSpec with Matchers:
  ...
  object TestStorage extends Storage[(UserData, EnrichedUserData), ((UserData, EnrichedUserData), EnrichedUserData)]:

    def store(
        bookkeepingResult: (UserData, EnrichedUserData),
        data: EnrichedUserData): ((UserData, EnrichedUserData), EnrichedUserData) =
      (bookkeepingResult, data)
